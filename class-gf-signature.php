...
class GFSignature extends GFAddOn {
	...
	public function merge_tag_filter( $value, $merge_tag, $options, $field ) {
		...
		if ( $merge_tag == 'all_fields' && $field['type'] == 'signature' ) {
			...
			$value          = "<a href='$signature_path' target='_blank' title='" . __( 'Click to view', 'gravityformssignature' ) . "'><img width='100' src='$signature_path'></a>";
	...
	public function signature_entry_detail( $value, $field, $lead, $form ) {
		if ( $field['type'] == 'signature' && ! empty( $value ) ) {
			$signature_path = $this->get_signature_url( $value );
			$newvalue       = "<a href='$signature_path' target='_blank' title='" . __( 'Click to view', 'gravityformssignature' ) . "'><img width='100' src='$signature_path'></a>";
		...
		if ( RG_CURRENT_VIEW <> 'entry' && is_admin() ) {
			...
			if ( ! empty( $signature_filename ) ) {
				...
				if ( RG_CURRENT_VIEW == 'entry' && $value ) {
					...
					$input .= "       <a href='" . $this->get_signature_url( $signature_filename ) . "' target='_blank' alt='" . __( 'Download file', 'gravityformssignature' ) . "' title='" . __( 'Download file', 'gravityformssignature' ) . "'><img src='" . GFCommon::get_base_url() . '/images/download.png' . "' /></a>" .
					          "       <a href='javascript:void(0);' alt='" . __( 'Delete file', 'gravityformssignature' ) . "' title='" . __( 'Delete file', 'gravityformssignature' ) . "' onclick='deleteSignature(" . $lead_id . ', ' . $field['id'] . ");' ><img src='" . GFCommon::get_base_url() . '/images/delete.png' . "' style='margin-left:8px;'/></a>";
				} else {
